 ...
 * @brief Contains definitions for buffered input handling and actions based off
 * raylib
 */
...
namespace raylib {
  ...
  /**
   * @brief Represents various input button types, including keyboard keys,
   * mouse buttons, and gamepad buttons.
   */
  struct Button {
    // Enumerates different button types.
    enum class Type { Invalid = 0, Keyboard, Mouse, Gamepad } type;

    // Union to store different types of button configurations (raylib enums).
    // NOTE: it is recommended to not directly manipulate buttons but to instead
    // use one of the following factory functions
    union {
     ...
     * @return True if this button is less than the given button, false
     * otherwise.
     */
    ...
    // Static helper functions to create Button objects for different input
    // types.
    /**
  ...
  /**
   * @brief Represents various input action types, such as buttons, axes
   * (controller trigger, mouse wheel), vectors (mouse, thumb stick), and
   * multi-button combinations.
   */
  struct Action {
    // Enumerates different action types.
    enum class Type { Invalid = 0, Button, Axis, Vector, MultiButton } type;
     ...
     */
    template <size_t N> struct MultiButtonData {
      enum Direction {
        Up,
        Down,
        Left,
      ...
      bool normalize =
          true; // When true the maximum value returned for a given axis is 1,
                // if false the value of each direction will be the sum of the
                // buttons pressed pointing one direction minus the sum of the
                // pressed buttons pointing the other direction
    };

    // Union to store different types of action data.
    // NOTE: It is recommended that you don't try to mess with these values
    // yourself, instead use one of the factory functions below NOTE: Every
    // inner type has a variable called last_state which holds the state as of
    // the last time this action was pumped!
    //	Can be quired externally if nessicary... but ensure that you are
    // accessing data of the correct type!
    union Data {
      struct Button {
        ButtonSet* buttons;
        bool combo = false; // When true all buttons in the set must be pressed
                            // for the action to trigger
        uint8_t last_state =
            0; // TODO: can we add a function to access the last state?
      } button;

      struct Axis {
        enum class Type { Invalid = 0, Gamepad, MouseWheel } type;
    ...
    ~Action() {
      if (type == Type::Button && data.button.buttons)
        delete data.button.buttons;
      else if (type == Type::MultiButton && data.multi.quadButtons)
    ...
    Action(Action&& o) noexcept : Action() {
      *this = std::move(o);
    }
     ...
     * Callback signature: [](const std::string_view name, uint8_t pressed, bool
     * wasPressed) -> void; The value in pressed represents how many buttons in
     * the set are currently pressed
     *
     * @param button the button to listen for
     * @param combo wether all of the buttons in the set need to be pressed for
     * a trigger (defaults to only a single button needing to be pressed)
     * @return Action
     */
    static Action button(Button button, bool combo = false) {
      return Action{Action::Type::Button,
                    Action::Data{.button = {new ButtonSet{button}, combo}}};
    }
    ...
    static Action key(KeyboardKey key, bool combo = false) {
      return button({Button::Type::Keyboard, {key}}, combo);
    }
    ...
    static Action mouse_button(MouseButton b, bool combo = false) {
      return button({Button::Type::Mouse, {.mouse = b}}, combo);
    }
     ...
     * @note same as pad
     */
    static Action gamepad_button(GamepadButton b, int gamepad = 0,
                                 bool combo = false) {
      return pad(b, gamepad, combo);
    }

    /**
     * @brief Action that is invoked whenever the one of the collection of
     * buttons are pressed. Callback signature: [](const std::string_view name,
     * uint8_t pressed, bool wasPressed) -> void; The value in pressed
     * represents how many buttons in the set are currently pressed
     *
    ...
    static Action button_set(ButtonSet buttons = {}, bool combo = false) {
      return Action{Action::Type::Button,
                    Action::Data{.button = {new ButtonSet(buttons), combo}}};
    }

    /**
     * @brief Action that is invoked whenever the gamepad axis (usually
     * triggers) is de/pressed. Callback signature: [](const std::string_view
     * name, float value, float delta) -> void
     *
     * @param axis the id of axis to listen for
     * @param gamepad id of the gamepad to listen for (default 0)
     * @return Action
     */
    static Action gamepad_axis(GamepadAxis axis = GAMEPAD_AXIS_LEFT_X,
                               int gamepad = 0) {
      return {Action::Type::Axis,
              {.axis = {Data::Axis::Type::Gamepad, {gamepad, axis}}}};
    }

    /**
     * @brief Action that is invoked whenever the mouse wheel is adjusted.
     * Callback signature: [](const std::string_view name, float value, float
     * delta) -> void
     *
     * @return Action
     */
    ...
    /**
     * @brief Action that combines button sets pointing in 2 opposing directions
     * into a signed value which represents the direction of the currently
     * pressed buttons. Callback signature: [](const std::string_view name,
     * float dir, float delta) -> void
     *
     ...
     * value will grow to reflect how many buttons are pushed. While true the
     * absolute value will never excede 1.
     * @return Action
     */
    static Action button_axis(ButtonSet positive, ButtonSet negative,
                              bool normalize = true) {
     ...
     * @return Action
     */
    static Action button_pair(ButtonSet left, ButtonSet right,
                              bool normalize = true) {
      return button_axis(left, right, normalize);
    }

    /**
     * @brief Action that is invoked whenever the mouse wheel is adjusted (also
     * supports wheels that can tilt side to side). Callback signature: [](const
    ...
    static Action mouse_wheel_vector() {
      return {Action::Type::Vector,
              {.vector = {Data::Vector::Type::MouseWheel}}};
    }

    /**
     * @brief Action that is invoked whenever the mouse moves.
     * Callback signature: [](const std::string_view name, Vector2 dir, Vector2
     * delta) -> void
     *
     * @return Action
     */
     ...
     * @param gamepadHorizontal id of the gamepad to listen for in the
     * horizontal axis (default 0)
     * @param gamepadVertical id of the gamepad to listen for in the vertical
     * axis. If its value is less than 0, it will copy the horizontal axis
     * (default -1)
     * @return Action
     */
    static Action gamepad_axes(GamepadAxis horizontal = GAMEPAD_AXIS_LEFT_X,
     ...
     * std::move but can be chained for convenience. This function is nessicary
     * since delegates (and thus actions) can't be copied, only moved.
     * @note After calling this function "this" will have been moved into the
     * reciever and shouldn't be accessed anymore!
     *
    ...
    /**
     * @brief Function which updates the state of the action and invokes the
     * callback if a change occured.
     * @note Automatically called by BufferedInput so there usually isn't a need
